...
namespace
{
	...
	bool gOrthographic = false;  // false = perspective, true = orthographic


	// edge-trigger helpers so O/P only toggle once per key press
	bool gPrevOState = false;
	bool gPrevPState = false;
}
 ...
 *
 *  This method is automatically called from GLFW whenever
 *  the scroll is moved within the active GLFW display window.
 ***********************************************************/
void ViewManager::Mouse_Scroll_Callback(GLFWwindow* window, double xOffset, double yOffset)
{
	if (g_pCamera != nullptr)
	{
		// Scroll up (positive yOffset) increases speed; down decreases
		movementSpeed += static_cast<float>(yOffset);
		movementSpeed = glm::clamp(movementSpeed, minSpeed, maxSpeed);
		g_pCamera->MovementSpeed = movementSpeed;
	}
}
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	if (oPressed && !prevO) {
		...
		// Reset camera for orthographic view
		g_pCamera->Position = glm::vec3(0.0f, 2.0f, 15.0f);
